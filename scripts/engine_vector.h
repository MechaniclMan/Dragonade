...
public:
	...
	DynamicVectorClass(DynamicVectorClass<T>&& vector): VectorClass<T>(static_cast<VectorClass<T>&&>(vector)), GrowthStep(vector.GrowthStep), ActiveCount(vector.ActiveCount)
	{
		/* nothing */
	}
	...
	DynamicVectorClass<T>& operator=(DynamicVectorClass<T>&& vector)
	{
		if (this != &vector)
		{
			VectorClass<T>::operator =(static_cast<VectorClass<T>&&>(vector));
...
{
public:
	inline PointerStack(T* initial_item, int initial_size = 63)
	{
		m_pStack = (T**)AllocateMemory(initial_size * sizeof(T*));
		m_pStack[0] = initial_item;
		m_iDepth = 1;
		m_iThreshold = initial_size;
	}
	...
	{
		if (m_iDepth == m_iThreshold)
		{
			m_iThreshold = m_iThreshold * 2;
			m_pStack = (T**)ReallocateMemory(m_pStack, m_iThreshold * sizeof(T*));
		}
...
public:
...
#if _MSC_VER >= 1600
	/* move semantics ftw */
	SimpleDynVecClass(SimpleDynVecClass<T>&& vector): SimpleVecClass(static_cast<SimpleVecClass<T>&&>(vector)), ActiveCount(vector.ActiveCount)
	{
		/* nothing */
	};
	...
	SimpleDynVecClass<T>& operator =(SimpleDynVecClass<T>&& vector)
	{
		if (this != &vector)
		{
			SimpleVecClass::operator =(static_cast<SimpleVecClass<T>&&>(vector));
			ActiveCount = vector.ActiveCount;
		}
		return *this;
	}
...
public:
	...
	T* Allocate_Object_Memory()
	{
		FastCriticalSectionClass::LockClass lock(ObjectPoolCS);
 		if (!FreeListHead)
		{
			uint32* newBlockListHead = (uint32*)((uint32*)new char[sizeof(uint32*) + sizeof(T[BLOCK_SIZE])]);
