...
ChunkLoadClass2::ChunkLoadClass2(FileClass *file)
{
	File = file;
	StackIndex = 0;
	memset(HeaderStack,0,sizeof(HeaderStack));
...
bool ChunkLoadClass2::Open_Chunk()
{
	if ((StackIndex <= 0) || (PositionStack[StackIndex-1] != (HeaderStack[StackIndex-1].ChunkSize & 0x7FFFFFFF)))
	{
		if (File->Read(&HeaderStack[StackIndex],8) == 8)
...
bool ChunkLoadClass2::Peek_Next_Chunk(unsigned int *id, unsigned int *length)
{
	ChunkHeader h;
	if ((StackIndex <= 0) || (PositionStack[StackIndex-1] != (HeaderStack[StackIndex-1].ChunkSize & 0x7FFFFFFF)))
	{
...
bool ChunkLoadClass2::Close_Chunk()
{
	unsigned int x = HeaderStack[StackIndex-1].ChunkSize & 0x7FFFFFFF;
	if (PositionStack[StackIndex-1] < (x))
	{
...
unsigned long ChunkLoadClass2::Cur_Chunk_ID()
{
	return HeaderStack[StackIndex-1].ChunkType;
}

unsigned long ChunkLoadClass2::Cur_Chunk_Length()
{
	return HeaderStack[StackIndex-1].ChunkSize & 0x7FFFFFFF;
}

int ChunkLoadClass2::Cur_Chunk_Depth()
{
	return StackIndex;
}

int ChunkLoadClass2::Contains_Chunks()
{
	return HeaderStack[StackIndex-1].ChunkSize & 0x80000000;
}

bool ChunkLoadClass2::Open_Micro_Chunk()
{
	int bytesread = Read(&MCHeader,sizeof(MCHeader));
	if (bytesread == sizeof(MCHeader))
	{
...
bool ChunkLoadClass2::Close_Micro_Chunk()
{
	InMicroChunk = false;
	if (MicroChunkPosition < 0)
	{
...
unsigned long ChunkLoadClass2::Cur_Micro_Chunk_ID()
{
	return MCHeader.ChunkType;
}

unsigned long ChunkLoadClass2::Cur_Micro_Chunk_Length()
{
	return MCHeader.ChunkSize;
}

long ChunkLoadClass2::Seek(unsigned long nbytes)
{
	if ((PositionStack[StackIndex-1] + nbytes) <= (HeaderStack[StackIndex-1].ChunkSize & 0x7FFFFFFF))
	{
		if ((!InMicroChunk) || ((MicroChunkPosition + nbytes) <= MCHeader.ChunkSize))
...
long ChunkLoadClass2::Read(void *buf, unsigned long nbytes)
{
	if ((PositionStack[StackIndex-1] + nbytes) <= (HeaderStack[StackIndex-1].ChunkSize & 0x7FFFFFFF))
	{
		if ((!InMicroChunk) || ((MicroChunkPosition + nbytes) <= MCHeader.ChunkSize))
...
long ChunkLoadClass2::Read(IOVector2Struct *v)
{
	return Read(v,sizeof(IOVector2Struct));
}

long ChunkLoadClass2::Read(IOVector3Struct *v)
{
	return Read(v,sizeof(IOVector3Struct));
}

long ChunkLoadClass2::Read(IOVector4Struct *v)
{
	return Read(v,sizeof(IOVector4Struct));
}

long ChunkLoadClass2::Read(IOQuaternionStruct *q)
{
	return Read(q,sizeof(IOQuaternionStruct));
}

long ChunkLoadClass2::Read(StringClass& string)
{
	uint16 length;
	int result = SimpleRead(length);
	result += Read(string.Get_Buffer(length+1), length);
...
ChunkSaveClass2::ChunkSaveClass2(FileClass *file)
{
	File = file;
	StackIndex = 0;
	memset(HeaderStack,0,sizeof(HeaderStack));
...
bool ChunkSaveClass2::Begin_Chunk(unsigned long id)
{
	ChunkHeader chunkh;
	chunkh.ChunkSize = 0;
	chunkh.ChunkType = 0;
...
bool ChunkSaveClass2::End_Chunk()
{
	ChunkHeader chunkh;
	int temp = File->Seek(0,1);
	StackIndex--;
...
int ChunkSaveClass2::Cur_Chunk_Depth()
{
	return StackIndex;
}

bool ChunkSaveClass2::Begin_Micro_Chunk(unsigned long id)
{
	MCHeader.ChunkType = (unsigned short)id;
	MicroChunkPosition = File->Seek(0,1);
	MCHeader.ChunkSize = 0;
	if (Write(&MCHeader,sizeof(MCHeader)) == sizeof(MCHeader))
	{
...
bool ChunkSaveClass2::End_Micro_Chunk()
{
	int temp = File->Seek(0,1);
	File->Seek(MicroChunkPosition,0);
	if (File->Write(&MCHeader,sizeof(MCHeader)) == sizeof(MCHeader))
...
unsigned long ChunkSaveClass2::Write(void* buf,unsigned long nbytes)
{
	if (File->Write(buf,nbytes) == (int)nbytes)
	{
		int temp = (HeaderStack[StackIndex-1].ChunkSize & 0x7FFFFFFF) + nbytes;
...
unsigned long ChunkSaveClass2::Write(IOVector2Struct& v)
{
	return Write(&v,sizeof(IOVector2Struct));
}

unsigned long ChunkSaveClass2::Write(IOVector3Struct& v)
{
	return Write(&v,sizeof(IOVector3Struct));
}

unsigned long ChunkSaveClass2::Write(IOVector4Struct& v)
{
	return Write(&v,sizeof(IOVector4Struct));
}
